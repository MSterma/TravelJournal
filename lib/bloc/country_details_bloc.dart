...
class CountryDetailsBloc extends Bloc<CountryDetailsEvent, CountryDetailsState> {
  CountryDetailsBloc(this.localRepo) : super(DetailsLoading()) {
  ...
  Future<void> _onLoadDetails(LoadDetails event, Emitter<CountryDetailsState> emit) async {
    emit(DetailsLoading());
    try {
      final isVisited = await localRepo.checkVisited(event.countryName);
      final photos = await localRepo.getPhotos(event.countryName);
